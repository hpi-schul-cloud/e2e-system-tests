...
function ccOrganizationHasWebcontent(directory, orgId, pattern) {
  return new Promise(async (resolve, reject) => {
    ...
    if (!new RegExp(pattern).test(filePath)) {
      reject(
        new Error(`Filepath ${filePath} of webcontent does not match expected pattern`)
      );
    }
...
function ccOrganizationHasWeblink(directory, orgId, url, title) {
  return new Promise(async (resolve, reject) => {
    ...
    if (webLinkInfo.url !== url) {
      reject(
        new Error(`Actual url ${webLinkInfo.url} does not match expected url ${url}`)
      );
    }
    if (webLinkInfo.title !== title) {
      reject(
        new Error(`Actual url ${webLinkInfo.title} does not match expected url ${title}`)
      );
    }
...
function getResourceType(doc, identifierRef) {
  const type =
    doc(`manifest > resources > resource[identifier="${identifierRef}"]`)?.attr("type") ||
    "";
  return type;
}
