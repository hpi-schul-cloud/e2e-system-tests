Feature: Course - Copy CTL tools
  ...
  @unstable_test
  Scenario Outline: Pre-test: Admin adds external tools to school
    Given I am logged in as a 'admin1_nbc' at 'nbc'
    When I go to administration page
    When I go to school administration
    When I click on external tools panel
    When I click the add external tool button
    When I select the tool '<tool_name>' from available tools
    When I click on save external tool button
    When I click on external tools panel
    Then I see the tool '<tool_name>' in external tools table
    Examples:
      | tool_name                                 |
  ...
  @unstable_test
  Scenario: Pre-test: Teacher creates a new course
  ...
  @unstable_test
  Scenario: Pre-test: Teacher adds three ctl tools to course
    When I go to rooms overview
...
#    Then I see the tool 'CY Test Tool Optional Protected Parameter' is not marked as incomplete operational
  ...
  @unstable_test
  Scenario Outline: Post-test: Teacher deletes course as a post condition
    When I go to rooms overview
