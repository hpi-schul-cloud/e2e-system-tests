...
Cypress.Commands.add('login', (username, environment) => {
  cy.session([username, environment], () => {
    ...
    if (environmentUpperCased === 'DEFAULT') {
      cy.visit('/login');
    }
    ...
    if (doExternalLogin){
      ...
      cy.get(externalUsernameInputFieldElement).type(env[userEmail], {log:false})
      cy.get(externalPasswordInputFieldElement).type(env[userPassword], {log:false}).type('{enter}')
    } else {
