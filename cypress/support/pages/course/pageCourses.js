...
class Courses {
	...
	static #contentCardContent = '[data-testid="task-title-0"]';
	static #taskCardThreeDotInRoom = '[data-testid="task-card-menu-0"]';
	...
	static #roomExternalToolSection =
	...
	static #studentGroupNameOnStudentGroupPage =
	...
	static #topicCardThreeDotInRoom = '[data-testid="lesson-card-menu-0"]';
	static #topicNameInTopicCardInRoom = '[data-testid="lesson-name-0"]';
	...
	openThreeDotMenuForContent(contentTitle) {
		...
		cy.get(Courses.#taskCardThreeDotInRoom).click();
	}

	openThreeDotMenuForTopic(contentTitle) {
		cy.get(Courses.#topicNameInTopicCardInRoom).contains(contentTitle);
		cy.get(Courses.#topicCardThreeDotInRoom).click();
	}
	...
	clearSubstituteTeacherField() {
		cy.get(Courses.#addSubstituteTeacher)
			.click()
	...
	clickThreeDotMenuOnTool(toolName) {
		...
		toolData
			.parent('div')
			.find(Courses.#threeDotMenuOnTool)
			.click()
			.should('exist');
	}
	...
	checkIfToolIsVisible(toolName) {
		cy.get(Courses.#toolConfigurationSelectItem).should(
			'not.contain',
			toolName
		);
	}
