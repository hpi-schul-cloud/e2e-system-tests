...
class Courses {
  ...
  static #dropDownCourse = '[data-testid="room-tool-three-dot-button"]'
  ...
  seeCopyResultNotification() {
    ...
    cy.get(Courses.#dialogTitle).siblings('div').eq(0).find('p')
        .invoke('text').should('have.length.gt', 0)
  }
  ...
  seeToolIsMarkedAsIncomplete(toolName) {
    const toolCard = cy.get(Courses.#roomExternalToolSection).find('div').contains(toolName)
    toolCard.parent('div').siblings('div').find(Courses.#incompleteChip).should('exist')
  }

  seeToolIsNotMarkedAsIncomplete(toolName) {
    const toolCard = cy.get(Courses.#roomExternalToolSection).find('div').contains(toolName)
    toolCard.parent('div').siblings('div').find(Courses.#incompleteChip).should('not.exist')
  }
