...
class Files {
  ...
  static #submitNewFolderInCourseFilesButton = '[data-testid="btn-submit-Neuer Ordner"]';
    ...
    cy.get("img#picture, video, audio") // data-testid is not available for media previews
      ...
      .each(($el) => {
        ...
        if (tag === "img") {
          ...
          if (alt) expect(alt, "Alt text should contain file name").to.include(fileName);
        } else if (["video", "audio"].includes(tag)) {
          cy.wrap(el).should(($media) => {
            // Wait until metadata is available
            expect($media[0].readyState, `${tag} metadata loaded`).to.be.greaterThan(0);
            expect($media[0].duration, `${tag} has duration`).to.be.greaterThan(0);
          });
        }
      });
  }
