...
class Management {
	...
	clickOnFAB(role) {
		const addUserFab =
			role === "student" ? Management.#addStudentFab : Management.#addTeacherFab;
	...
	seeTheAssignedBirthDateInUserTable() {
		cy.get("@assignedBirthDate").then((assignedBirthDate) => {
			...
			cy.get(Management.#tableContents)
				...
				.invoke("text")
				.then((tableText) => {
					expect(normalizeDate(tableText)).to.contain(normalizeDate(assignedBirthDate));
				});
		});
	}
	...
	// 			.should("be.visible")
	// 			.find("tbody tr")
	// 			// wait until the loading row is gone
	// 			.should(($rows) => {
	// 				const loadingRow = [...$rows].some((r) =>
	// 					r.textContent.includes("Daten werden geladen")
	// 				);
	// 				expect(loadingRow).to.be.false;
	// 			})
	// 			.then(($rows) => {
	// 				// check empty state
	// 				if (
	...
	// 						(el) => el.textContent.trim() === toolName
	// 					);
	...
	showAllExternalToolsInTable() {
		cy.get(Management.#externalToolsTable).scrollIntoView().should("be.visible");

		// Select "Alle" to show all items on one page
		cy.get(Management.#externalToolsTable)
			.find(".v-data-table-footer__items-per-page .v-select")
			.click();
		cy.get(".v-list-item").contains("Alle").click();
		cy.wait(500);

		// cy.get(Management.#externalToolsTable)
		// 	.find(".v-data-table-footer__info")
		// 	.should("not.contain.text", "0-0 von 0");

		// Wait for table to finish loading after changing page size
		cy.get(Management.#externalToolsTable)
			.find("tbody tr", { timeout: 80_000 })
			.should(($rows) => {
				const loadingRow = [...$rows].some((r) =>
					r.textContent.includes("Daten werden geladen")
				);
