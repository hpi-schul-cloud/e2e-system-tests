@news @stable_test
Feature:  News - To read a news on the respective dashboards

  As a teacher I want to read the news shown on the dashboard so that I'm informed about the latest news

  Scenario: as a pre-condition teacher creates school news
    Given I am logged in as a 'teacher' at 'brb'
    When I go to news overview
    And I click on add news button
    Then I see news creation page
    And I enter news title 'Read news - this is a school news'
    And I enter news description 'test school news description'
    And I see date input field
    And I see time input field
    And I click on save button
    Then I see news is created successfully with title 'Read news - this is a school news' and with description 'test school news description'

  Scenario: as a pre-condition teacher creates a team news
    Given I am logged in as a 'teacher' at 'brb'
    When I go to teams overview
    And I go to a team 'Musik'
    When I click on news tab on the team detail page
    And I click on create news button
    Then I see news creation page
    And I enter news title 'Read news - this is a team news'
    And I enter news description 'test team news description'
    And I see date input field
    And I see time input field
    And I click on save button
    Then I see news is created successfully with title 'Read news - this is a team news' and with description 'test team news description'

  Scenario: Reading a school news on news overview page
    Given I am logged in as a 'teacher' at 'brb'
    When I go to news overview
    Then I can read the news 'Read news - this is a school news' with description 'test school news description'

  Scenario: Reading a team news on teams news overview page
    Given I am logged in as a 'teacher' at 'brb'
    When I go to teams overview
    And I go to a team 'Musik'
    When I click on news tab on the team detail page
    Then I can read the news 'Read news - this is a team news' with description 'test team news description'

  Scenario: as a post-condition teacher deletes the school news
    Given I am logged in as a 'teacher' at 'brb'
    When I go to news overview
    And I click on the news teaser 'Read news - this is a school news'
    When I click on delete button
    And I confirm the deletion on confirmation dialog box
    Then I do not see the news 'Read news - this is a school news'

  Scenario: as a post-condition teacher deletes the team news
    Given I am logged in as a 'teacher' at 'brb'
    When I go to news overview
    And I click on the news teaser 'Read news - this is a team news'
    When I click on delete button
    And I confirm the deletion on confirmation dialog box
    Then I do not see the news 'Read news - this is a team news'